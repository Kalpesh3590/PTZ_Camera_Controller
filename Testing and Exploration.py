...
class CompactPTZRemote:
    ...
    def save_settings_from_window(self) -> None:
        ...
        if theme_changed:
            ...
            opacity = float(settings["general"]["opacity"])
            nearest = min(range(len(OPACITY_VALUES)), key=lambda index: abs(OPACITY_VALUES[index] - opacity), )
            self.opacity_index = nearest
    ...
    def controller_pointer_enter(self, _event: tk.Event | None = None) -> None:
        if not self.is_closing:
            self.root.attributes("-alpha", min(1.0, OPACITY_VALUES[self.opacity_index] + float(
                self.user_settings["general"]["hover_boost"])))
